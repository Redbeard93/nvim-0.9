...
set.edit = require("V9.plugins.config.telescope.actions.set").edit

local previewers = require("telescope.previewers")
previewers.buffer_previewer_maker = require("V9.plugins.config.telescope.buffer_previewer").file_maker

local conf = require("telescope.config").values
conf["qflist_previewer"] = function(...)
  return require("V9.plugins.config.telescope.buffer_previewer").qflist.new(...)
end
telescope.setup({
  defaults = {
    vimgrep_arguments = {
    ...
    preview = {
      ...
      filetype_hook = function(filepath, bufnr, opts)
        if vim.startswith(filepath, "jdt://") then
          require("V9.lsp.utils.jdtls").open_classfile(filepath, bufnr, opts.preview.timeout)
          return false
        end
        return true
      end,
